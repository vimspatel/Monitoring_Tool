...
$error_location = "D:\MonitoringScripts\Monitor_SP\Error_bin\output_services_error.txt"
########################################################################################################

################################## Get Service Status Function #########################################

function servicestatus ($id,$description,$serverlist,$service)
{
...
#call the invoke-sqlcmdlet to execute the query
         ExecuteSqlQuery -sqlquery $InsertResults
}
...
$resultsDataTable = ExecuteSqlQuery -sqlquery 'SELECT * FROM VP_Server_ENV_Detail a INNER JOIN VP_Service_List b on a.Env_ID = b.Env_ID' 
...
{
#Call Function
servicestatus $report_service_id $report_Description $report_Server_Name $report_Service_Name 
}
